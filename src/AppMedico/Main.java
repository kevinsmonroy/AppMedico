...
public class Main {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            List<Usuario> usuarios = PersistenciaBinaria.cargarLista("usuarios.bin");
            if (usuarios == null) usuarios = new ArrayList<>();

            GestorCitas gestor = PersistenciaBinaria.cargarObjeto("citas.bin");
            if (gestor == null) gestor = new GestorCitas();

            // Si NO hay datos, crear de prueba (solo la primera vez)
            if (usuarios.isEmpty()) {
                // Crear médico
                ...
                gestor.getTodasLasCitas().get(1).setEstado(EstadoCita.ATENDIDA);
                gestor.getTodasLasCitas().get(2).setEstado(EstadoCita.INASISTENCIA);

                // Guardar datos iniciales
                PersistenciaBinaria.guardarLista(usuarios, "usuarios.bin");
                PersistenciaBinaria.guardarObjeto(gestor, "citas.bin");
            }
            ...
            // Mostrar listado de citas por fecha con edad del paciente
            ...
            // Al cerrar la app o después de realizar cambios, guarda los datos nuevamente:
            // PersistenciaBinaria.guardarLista(usuarios, "usuarios.bin");
            // PersistenciaBinaria.guardarObjeto(gestor, "citas.bin");
        });
    }
